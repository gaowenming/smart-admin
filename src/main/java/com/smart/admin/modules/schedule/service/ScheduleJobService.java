...
public class ScheduleJobService {
    ...
    public void add(ScheduleJob scheduleJob) {
        ...
        CronTrigger trigger = TriggerBuilder.newTrigger().withIdentity(scheduleJob.getName(), scheduleJob.getGroup()).withSchedule(scheduleBuilder.withMisfireHandlingInstructionDoNothing()).build();
        try {
            ...
            for (JobKey jobKey : jobKeys) {
                ...
                for (Trigger trigger : triggers) {
                    ...
                    if (trigger instanceof SimpleTrigger) {
                        ...
                        scheduleJob.setCronExpression("重复次数:" + (simple.getRepeatCount() == -1 ? "无限" : simple.getRepeatCount()) + ",重复间隔:" + (simple.getRepeatInterval() / 1000L));
